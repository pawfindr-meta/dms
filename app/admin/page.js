...
export default function AdminPage() {
  ...
  const handleSaveTeam = async (e) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data?.error || 'Failed to deploy team');
  ...
  const handleDeleteTeam = async (team) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data?.error || 'Decommission failed');
  ...
  const handleCreateTech = async (e) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data?.error || 'Failed to enroll operative');
  ...
  const handleResetPin = async (id) => {
    if (!confirm(`Restore default PIN 00000000 for ${id}?`)) return;
    try {
      const res = await fetch(`/api/technicians/${id}/reset-pin`, { method: 'POST' });
      const data = await res.json();
      if (!res.ok) throw new Error(data?.error || 'Failed to restore default PIN');
  ...
  return (
    <div className="h-screen w-screen bg-black text-white flex flex-col overflow-hidden font-sans">
      ...
      <main className="flex-1 min-h-0 w-full p-6 md:p-8 flex flex-col gap-6">
        {/* Navigation Ribbon */}
        <div className="flex flex-wrap items-center justify-between border-b border-white/10 pb-4 gap-4 shrink-0">
          <div className="flex gap-3">
            {[
              ...
              <button
                ...
                className={`px-6 py-3 text-xs uppercase tracking-[0.3em] font-black border transition-all cursor-pointer ${
                  activeTab === tab.id
        ...
        {message && (
          <div className="p-3.5 border border-white/20 bg-white/[0.02] text-sm font-mono uppercase tracking-widest text-emerald-400 shrink-0">
            [SYS_MSG] {message}
          </div>
        )}
        ...
        {activeTab === 'users' && (
          <div className="flex-1 min-h-0 flex flex-col border border-white/10 bg-white/[0.01]">
            <div className="p-6 flex justify-between items-center border-b border-white/10 shrink-0">
              <div>
                <span className="text-[11px] font-mono uppercase tracking-[0.3em] text-white/40">Console Accounts</span>
                <h2 className="text-xl font-black uppercase tracking-tight">System Staff Directory ({users.length})</h2>
              </div>
              <button
                onClick={() => {
                  setEditingUser(null);
                ...
                className="px-6 py-3 bg-white text-black text-xs font-black uppercase tracking-[0.25em] hover:bg-white/80 cursor-pointer"
              >
                + Register User
              </button>
            </div>
                ...
                <tbody className="divide-y divide-white/10">
                  {users.map((u) => (
                    <tr key={u.user_id} className="hover:bg-white/[0.03] transition">
                      ...
                      <td className="p-4 font-bold text-white uppercase text-base">{u.full_name}</td>
                      <td className="p-4 text-white/80 text-sm">{u.username_or_email}</td>
                      <td className="p-4">
                      ...
                      <td className="p-4 text-right space-x-3">
                        <button
                          ...
                          className="px-4 py-1.5 border border-white/20 hover:bg-white hover:text-black transition text-xs uppercase tracking-wider font-bold cursor-pointer"
                        >
                          Configure
                        </button>
                        <button
                          onClick={() => handleDeleteUser(u)}
                          className="px-4 py-1.5 border border-red-500/40 text-red-400 hover:bg-red-500 hover:text-black transition text-xs uppercase tracking-wider font-bold cursor-pointer"
                        >
                          Revoke
                        </button>
                      </td>
        ...
        {activeTab === 'teams' && (
          <div className="flex-1 min-h-0 flex flex-col space-y-4">
            <div className="flex justify-between items-center shrink-0">
              <div>
                <span className="text-[11px] font-mono uppercase tracking-[0.3em] text-white/40">Field Deployment Units</span>
                <h2 className="text-xl font-black uppercase tracking-tight">Active Team Matrix ({teams.length})</h2>
              </div>
              <button
                onClick={() => {
                  setEditingTeam(null);
            ...
            <div className="flex-1 min-h-0 overflow-y-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pr-2">
              {teams.map((t) => (
                <div
                  key={t.team_id}
                  className="p-6 border border-white/10 bg-white/[0.01] hover:border-white/40 transition flex flex-col justify-between space-y-6 h-fit"
                >
                  <div className="space-y-4">
                    <div className="flex justify-between items-start">
                      <h3 className="font-black text-white uppercase text-lg tracking-tight">{t.team_name}</h3>
                      <span className="px-3 py-1 border border-emerald-400/40 text-emerald-400 text-xs font-mono font-bold uppercase">
                        {t.active_workload || 0} In Field
                      </span>
                    </div>

                    <div>
                      <span className="text-[11px] font-mono uppercase tracking-[0.25em] text-white/40 block mb-2">
                        Assigned Operatives ({t.members?.length || 0})
                      </span>
                      <div className="space-y-2 max-h-40 overflow-y-auto">
                        {t.members?.length > 0 ? (
                          t.members.map((m, idx) => (
                            <div key={idx} className="p-2.5 border border-white/5 bg-white/[0.02] flex justify-between items-center text-sm font-mono">
                              <span className="text-white uppercase font-bold">{m.full_name}</span>
                              <span className="text-emerald-400 font-bold">{m.tech_id}</span>
                            </div>
                          ))
                        ) : (
                          <div className="text-sm font-mono text-white/30 py-2 uppercase">No operatives assigned</div>
                        )}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="pt-4 border-t border-white/10 flex justify-end gap-3">
                    <button
                      ...
                      className="px-4 py-2 border border-white/20 text-xs uppercase tracking-widest font-black hover:bg-white hover:text-black transition cursor-pointer"
                    >
                      Edit Roster
                    </button>
                    <button
                      onClick={() => handleDeleteTeam(t)}
                      className="px-4 py-2 border border-red-500/30 text-red-400 text-xs uppercase tracking-widest font-black hover:bg-red-500 hover:text-black transition cursor-pointer"
                    >
                      Decommission
                    </button>
                  </div>
        ...
        {activeTab === 'techs' && (
          <div className="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-12 gap-6">
            <div className="lg:col-span-4 border border-white/10 bg-white/[0.01] p-6 flex flex-col justify-between h-full overflow-y-auto">
              <div>
                <h3 className="text-base font-black uppercase tracking-tight text-white mb-4">Enroll Personnel</h3>
                <form onSubmit={handleCreateTech} className="space-y-4 font-mono">
                  <div>
                    <label className="block text-[11px] uppercase tracking-widest text-white/40 mb-1.5">Personnel Type</label>
                    <select
                      value={personnelType}
                  ...
                  <div>
                    <label className="block text-[11px] uppercase tracking-widest text-white/40 mb-1.5">Operative ID</label>
                    <input
                      type="text"
                      required
                      value={techId}
                  ...
                  <div>
                    <label className="block text-[11px] uppercase tracking-widest text-white/40 mb-1.5">Full Legal Name</label>
                    <input
                      type="text"
                      required
                      value={fullName}
                  ...
                  <div>
                    <label className="block text-[11px] uppercase tracking-widest text-white/40 mb-1.5">Contact Link</label>
                    <input
                      type="text"
                      value={contactNumber}
                  ...
                  <button
                    type="submit"
                    className="w-full py-3.5 bg-white text-black font-black uppercase text-xs tracking-widest hover:bg-white/80 transition cursor-pointer mt-2"
                  >
                    Enroll Operative
                  </button>
                </form>
            ...
            <div className="lg:col-span-8 border border-white/10 flex flex-col h-full overflow-hidden">
              <div className="flex-1 overflow-auto">
                <table className="w-full text-left text-sm font-mono">
                  <thead className="sticky top-0 bg-black/95 z-10">
                    <tr className="border-b border-white/10 text-[11px] uppercase tracking-[0.25em] text-white/40 bg-white/[0.02]">
                  ...
                  <tbody className="divide-y divide-white/10">
                    {techs.map((t) => (
                      <tr key={t.tech_id} className="hover:bg-white/[0.02]">
                        <td className="p-4 font-bold text-emerald-400 text-base">{t.tech_id}</td>
      ...
      {userModalOpen && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md flex items-center justify-center p-6 z-50">
          <div className="max-w-xl w-full border border-white/20 bg-black p-8 space-y-6 shadow-2xl">
            ...
            <form onSubmit={handleSaveUser} className="space-y-4 font-mono">
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-white/10">
                <button
                  ...
                  className="px-5 py-2.5 border border-white/20 text-xs uppercase tracking-widest text-white hover:bg-white/10"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={savingUser}
                  className="px-6 py-2.5 bg-white text-black text-xs font-black uppercase tracking-widest hover:bg-white/80"
                >
        ...
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md flex items-center justify-center p-6 z-50">
          <div className="max-w-xl w-full border border-white/20 bg-black p-8 space-y-6 shadow-2xl">
            ...
            <form onSubmit={handleSaveTeam} className="space-y-4 font-mono">
              ...
              />
              <div>
                <label className="block text-[11px] uppercase tracking-widest text-white/40 mb-2">Assign Personnel</label>
                <div className="max-h-60 overflow-y-auto border border-white/10 p-3 space-y-2">
                  {techs.map((t) => (
